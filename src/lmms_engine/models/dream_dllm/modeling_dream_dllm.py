...
@dataclass
class DreamDLLMMaskedLMOutput(MaskedLMOutput):
...
class DreamDLLMRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        DreamDLLMRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class DreamDLLMAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: DreamDLLMConfig, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        self.q_norm = DreamDLLMRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
        self.k_norm = DreamDLLMRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # thus post q_norm does not need reshape
...
class DreamDLLMDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: DreamDLLMConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = DreamDLLMAttention(config=config, layer_idx=layer_idx)

        self.mlp = DreamDLLMMLP(config)
        self.input_layernorm = DreamDLLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = DreamDLLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class DreamDLLMPreTrainedModel(PreTrainedModel):
    config: DreamDLLMConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["DreamDLLMDecoderLayer"]
    ...
    _can_record_outputs = {
        "hidden_states": DreamDLLMDecoderLayer,
        "attentions": DreamDLLMAttention,
    }


class DreamDLLMRotaryEmbedding(nn.Module):
    def __init__(self, config: DreamDLLMConfig, device=None):
        super().__init__()
...
class DreamDLLMModel(DreamDLLMPreTrainedModel):
    def __init__(self, config: DreamDLLMConfig):
        ...
        self.layers = nn.ModuleList(
            [DreamDLLMDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = DreamDLLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = DreamDLLMRotaryEmbedding(config=config)
...
class DreamDLLMForMaskedLM(DreamDLLMPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = DreamDLLMModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if reduction == "sum":
            ...
            d_loss = d_loss.mean()
            nll = nll.mean()
        return DreamDLLMMaskedLMOutput(
            loss=d_loss,
            nll=nll,
            logits=logits,
            hidden_states=hidden_states,
